...
public class ConlluTokenExpansionSamplesConverterTest {
    @Mock
    private ConlluNormalizer normalizer;
    @Mock
    private ConlluValidator validator;
    @InjectMocks
    private ConlluTokenExpansionSamplesConverter converter;
    ...
    public void convert() {
        ...
        assertThat(samples).isEqualTo(Vector.of(createSample(Vector.of(
                Tuple.of("Some", "X", "Somt mex"),
                Tuple.of("example", "X", "example"),
                Tuple.of("sen", "X", "sen"),
                Tuple.of("ten", "X", "ten"),
                Tuple.of("ce", "X", "ce"),
                Tuple.of("ten", "X", "ten"),
    ...
    private LemmaSample createSample(Seq<Tuple3<String, String, String>> segments) {
        return new LemmaSample(
                segments.map(Tuple3::_1).toJavaList(),
                segments.map(Tuple3::_2).toJavaList(),
                segments.map(Tuple3::_3).toJavaList());
    }
}
